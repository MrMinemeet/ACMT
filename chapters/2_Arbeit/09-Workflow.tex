\chapter{Workflow} \label{ref:workflow}
...
For running the DeepTAM a \href{https://support.lenovo.com/us/en/solutions/pd005642}{\textit{Lenovo Think-Station S20}} is used since it requires a dedicated graphics card. Since the DeepTAM is a bit tricky to get it working a \href{https://www.lenovo.com/de/de/laptops/thinkpad/w-series/w550s/}{\textit{Lenovo W550s}} was used as a secondary machine to increase the working progress. For more intense work a server access at the Johannes Kepler University was supplied to work on their system which contains Nvidia Quadro and Nvidia GTX graphics cards. \newline
...
The Raspberry Pi\texttrademark \footnote{Raspberry Pi is a trademark} is running \href{https://www.raspberrypi.org/downloads/raspbian/}{Raspbian Buster} since it is a well optimized version of Debian for the mini computer and only required to be able to execute a Python script to send the raw video feed using the Hypertext Transfer Protocol (\gls{http}) to the processing device.\newline
...
At first Ubuntu 16.04 with Automotive Data and Time-Triggered Framework (\gls{adtf}) was used since it's the recommended environment by the \gls{aadc} car manufacturer DigitalWerk. There were many compatibility an stability issues and it is very difficult to get into the whole system as it's not very beginner friendly. After trying to get the basics of \gls{adtf} working it was clear that switching to ROS might be better. The main problems with \gls{adtf} are, that \gls{adtf} isn't running very stable, requires certain packages to be in a non-standard folders and not having them in the regular location and it is very difficult to work with when using it for the first time.
...
Before using the ORB-SLAM the calibration file needs to be created in the ORB-\gls{slam}s config folder with the name \textit{config.yaml}. For a baseline the config from another mono-camera can be copied. The calibration part looks like shown in listing \ref{code:orbslamconfig} and thus the config that the calibration tool generated needs to be converted manually. In the camera\_matrix the values are the following:\newline
...
% ---------------------------------------------------
% Results
\section{Result with ORB-SLAM} \label{ref:orbslamresults}
...
\begin{lstlisting}[label={code:csparseincludedir}]
...
\subsubsection{Could not find package configuration file provided by \textsc{Eigen}}
...
\begin{itemize}
    ...
    The camera should have a global shutter where the the image is taken all at once. Rolling shutters which scan the image from top to bottom. This might lead to changes while the scan is running and thus generating false information. This wrong information then leads to inferior results.
...
\section{Launching}
When running the LSD-\gls{slam} it is necessary to specify what the sources are. The command should look similar to command \ref{code:lsdslamcorestart} and for debugging purpose it is also possible to show the debug image that the \gls{slam} is working on using the command. \ref{code:lsdslamviewerstart}.
\begin{lstlisting}[language=bash, label={code:lsdslamcorestart}]
    rosrun lsd_slam_core live_slam image:=/camera/image_raw camera_info:=/camera/camera_info
\end{lstlisting}
\begin{lstlisting}[language=bash, label={code:lsdslamviewerstart}]
    rosrun lsd_slam_viewer viewer
\end{lstlisting}
When looking in RQt \ref{rqt} it shows that the LSD-\gls{slam} has connected to the camera and the viewer connected to the LSD-\gls{slam}. The node graph most likely looks like shown in figure \ref{img:nodegraphlsdslam}.
\begin{figure}[h]
	\centering
	\includegraphics[width=1\textwidth]{./media/images/nodegraphlsdslam.png}
  	\caption{Node Graph with Stream and LSD-SLAM}
  	\label{img:nodegraphlsdslam}
\end{figure}

 \section{Results with LSD-SLAM}
The result when running the LSD-\gls{slam} is a 3D map which contains many dots in space which represent a edges, surfaces and other details. Compared to the ORB-\gls{slam} results \ref{ref:orbslamresults} the LSD-\gls{slam} produces a more detailed map in terms of amount of points and how they are laid out. When looking into a visualization programm like RViz \ref{rviz} the map should look similar to \ref{img:lsdslamresult} which shows the more detailed map due to more information gathered by the \gls{slam}.
\begin{figure}[h]
	\centering
	\includegraphics[height=0.7\textwidth]{./media/images/lsdslamresult.png}
  	\caption{LSD-SLAM Topdown and Detailed View
  	\\ Source: https://tinyurl.com/uepquna}
  	\label{img:lsdslamresult}
\end{figure}
...
In the following section the errors that occurred when trying to get DeepTAM to run will be listed alongside with the solutions.
...
This error already has been fixed from uni-freiburg and posted on both issues.
\begin{lstlisting}[language=bash]
#### Error Message
